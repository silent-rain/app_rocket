...
impl TokenApiAuth {
    // 获取所有 Token URI 列表
    pub fn get_all(conn: &DbConnection) -> QueryResult<Vec<TokenApiAuth>> {
    ...
    // 添加 Token URI
    pub fn insert(token_: &TokenApiAuthData, conn: &DbConnection) -> QueryResult<usize> {
    ...
    // 禁用/启用 Token URI
    pub fn update_status(token_uri: &TokenApiAuthData, conn: &DbConnection) -> QueryResult<usize> {
    ...
    // 更新 Token URI 有效期
    pub fn update_expire(token_uri: &TokenApiAuthData, conn: &DbConnection) -> QueryResult<usize> {
        diesel::update(token_api_auth::table.filter(token_api_auth::id.eq(token_uri.id)))
    ...
    // 删除 Token URI
    pub fn delete_by_id(id: i32, conn: &DbConnection) -> QueryResult<usize> {
